...
dedicated scene. It then shows violin, chord, contour, and market-depth scenes
with authentic animated Dither treatments generated by those new native
implementations. The creative contract, asset provenance, and exact reproduction
commands live in [`ADVANCED_PR_DEMO.md`](ADVANCED_PR_DEMO.md). Native inputs live
in `public/advanced-pr-light/`; the framing, reveals, typography, and scene
transitions are authored in `src/advanced-pr/`.
