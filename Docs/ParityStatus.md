...
The latest chart-only comparison, verified on 2026-07-11 against upstream commit `069899598a11e00094ea1eb6b838404825f828be`, is:

| Scenario | Changed pixels | Mean abs | RMS |
| --- | ---: | ---: | ---: |
| `line-orderbook` | 4.581% | 1.555 | 10.542 |
...
| `candle-wide-window` | 1.669% | 0.443 | 4.976 |
...
| `line-loading` | 1.353% | 0.446 | 4.671 |
| `candle-loading` | 1.322% | 0.327 | 2.966 |
| `line-empty` | 1.292% | 0.541 | 6.559 |
...
- `line-orderbook` is the largest remaining outlier, but it is below the accepted 5% changed-pixel threshold. Weighted label picks, seed, stream phase, and outlined SwiftUI text match the upstream reference without allocating platform bitmaps in the render loop; the remaining delta is mostly cross-platform text antialiasing and tiny motion-phase differences.
